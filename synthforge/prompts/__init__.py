...
# Agent instruction to consult DevOps best practices via MCP tools
DEVOPS_CONSULTATION = """

Consult available remote MCP tools BEFORE generating: MS Learn MCP (Azure documentation) and Bing Grounding (web search). Apply: file separation, naming patterns, config objects, dynamic generation based on researched best practices.
"""
from typing import Any, Optional


def _deep_merge_dicts(base: dict[str, Any], override: dict[str, Any]) -> dict[str, Any]:
    """Deep merge dictionaries with override values taking precedence."""
    result = dict(base)
...
def load_yaml_with_includes(path: Path) -> dict[str, Any]:
    """Load YAML file and merge any included YAML files (relative to the file path)."""
    if not path.exists():
    ...
    includes = base_data.get("includes", [])
    ...
    for include in includes:
        ...
        if not include_path.exists():
            raise FileNotFoundError(
                f"Included YAML file not found: {include_path} (included from {path})"
            )
        with open(include_path, "r", encoding="utf-8") as include_file:
            include_data = yaml.safe_load(include_file) or {}
        merged_data = _deep_merge_dicts(merged_data, include_data)

    base_data.pop("includes", None)
    return _deep_merge_dicts(merged_data, base_data)
...
def get_agent_instructions(agent_name: str) -> str:
    ...
    Supports both legacy key names (instructions) and new agent-specific key names
    ({agent_name}_instructions) for backward compatibility.
    
    Args:
        agent_name: The agent name (e.g., 'vision_agent', 'filter_agent')
    ...
    agent_specific_key = f"{agent_name}_instructions"
    if agent_specific_key in config:
        return config[agent_specific_key]
...
def get_user_prompt_template(agent_name: str) -> str:
    ...
    Supports both legacy key names (user_prompt_template) and new agent-specific key names
    ({agent_name}_user_prompt) for backward compatibility.
    ...
    # Try agent-specific key first (new pattern)
    agent_specific_key = f"{agent_name}_user_prompt"
    if agent_specific_key in config:
        return config[agent_specific_key]
...
def get_iac_user_prompt_template(agent_name: str) -> str:
    """
    Get the user prompt template for a specific IaC agent (Phase 2).
    ...
    if agent_name not in instructions_data:
        raise KeyError(f"Agent '{agent_name}' not found in iac_agent_instructions.yaml")

    config = instructions_data[agent_name]
...
def get_typed_instructions(agent_name: str, instruction_type: str) -> str:
    """
    Get type-specific instructions for agents that support multiple formats (e.g., Terraform/Bicep).
    
    Used by module_development_agent and deployment_wrapper_agent which have separate instructions
    for different IaC formats.
    
    Args:
        agent_name: The agent name (e.g., 'module_development_agent', 'deployment_wrapper_agent')
        instruction_type: The instruction type (e.g., 'terraform', 'bicep')
    
    Returns:
        Type-specific instructions string
    
    Examples:
        >>> get_typed_instructions('module_development_agent', 'terraform')
        >>> get_typed_instructions('module_development_agent', 'bicep')
        >>> get_typed_instructions('deployment_wrapper_agent', 'terraform')
    """
    # Load from iac_agent_instructions.yaml (these agents are IaC-specific)
    instructions_data = load_iac_instructions()
    ...
    # Try agent-specific key first (new pattern)
    # e.g., "module_development_agent_terraform_instructions"
    agent_specific_key = f"{agent_name}_{instruction_type}_instructions"
    if agent_specific_key in config:
        return config[agent_specific_key]
    
    # Fall back to generic typed key for backward compatibility
    # e.g., "terraform_instructions"
    generic_typed_key = f"{instruction_type}_instructions"
    return config.get(generic_typed_key, "")


def get_prompt_template(agent_name: str, template_key: str, from_iac: bool = False) -> str:
    """
    Get any prompt template from agent configuration.
    
    This is a generic getter for loading dynamic prompts that were moved from
    Python code to YAML configuration files.
    
    Args:
        agent_name: The agent name (e.g., 'deployment_wrapper_agent', 'description_agent')
        template_key: The template field name (e.g., 'categorization_prompt_template')
        from_iac: If True, load from iac_agent_instructions.yaml; otherwise from agent_instructions.yaml
    
    Returns:
        Template string with placeholders (e.g., {services_json})
    
    Raises:
        KeyError: If agent or template not found
        
    Examples:
        >>> get_prompt_template('deployment_wrapper_agent', 'categorization_prompt_template', from_iac=True)
        >>> get_prompt_template('description_agent', 'context_hints_template')
        >>> get_prompt_template('filter_agent', 'foundational_services_guidance')
        >>> get_prompt_template('code_quality_agent', 'validation_analysis_prompt_template', from_iac=True)
    """
    if from_iac:
        instructions_data = load_iac_instructions()
    else:
    ...
    if agent_name not in instructions_data:
        yaml_file = "iac_agent_instructions.yaml" if from_iac else "agent_instructions.yaml"
        raise KeyError(f"Agent '{agent_name}' not found in {yaml_file}")
    
    config = instructions_data[agent_name]
    
    if template_key not in config:
        raise KeyError(f"Template '{template_key}' not found in {agent_name} config")
...
def get_module_development_agent_instructions(iac_format: str = "terraform") -> str:
    ...
    Supports both legacy key names (terraform_instructions, bicep_instructions) and new agent-specific
    key names ({agent_name}_{format}_instructions) for backward compatibility.
    ...
    """
    # Use the new get_typed_instructions helper
    return get_typed_instructions("module_development_agent", iac_format.lower()) + DEVOPS_CONSULTATION
...
def get_deployment_wrapper_agent_instructions(iac_format: str = "terraform") -> str:
    ...
    """
    # Use the new get_typed_instructions helper
    return get_typed_instructions("deployment_wrapper_agent", iac_format.lower()) + DEVOPS_CONSULTATION
...
def get_iac_prompt_template(template_name: str) -> str:
